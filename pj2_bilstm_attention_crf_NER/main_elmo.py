...
from model_elmo import BiLSTM_CRF
from epoch_elmo import train, evaluate
...
# elmo
parser.add_argument('--elmo_options_file', type=str, default='./data/options.json',
                    help='path of the data corpus')
parser.add_argument('--elmo_weight_file', type=str, default='./data/elmo_2x4096_512_2048cnn_2xhighway_weights.hdf5',
                    help='path of the trained model')
...
## 模型架构
args.mode_char='elmo'#  (None, lstm, cnn, elmo) # None: 不使用mode char
...
if args.mode_char == 'elmo':
    args.dim_elmo=512*2 # 

args.dims=(dim_emb_char, dim_emb_word, dim_out_char, dim_out_word)
...
# get loader
from utils_elmo import load_sentences, update_tag_scheme, prepare_dataset
from torch.utils.data import DataLoader
from dataloader_elmo import CoNLLData, collate_fn
...
import torch
from epoch_elmo import  evaluate
...
def test_and_write(_data_iter,_data, type="test", model=model):
    ...
    if not write:
        return
    with open(f'model/{type}_res_{args.mode_char}_{args.mode_word}_{f1}_{args.enable_crf}.txt',encoding="utf-8", mode='w') as f:
        idx=0
