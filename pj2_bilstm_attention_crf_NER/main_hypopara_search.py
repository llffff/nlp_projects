...
## 模型架构
args.mode_char='cnn'#'lstm' #(None, lstm, cnn)
...
# dim_out_char = 0 # char representation 维度
# dim_out_word = 128 # word representation 维度
# args.dims=(dim_edb_char, dim_emb_word, dim_out_char, dim_out_word)

# args.window_kernel=5 # cnn 参数
...
def _fit(args):
    ...
    # print(f'\n[info] | lr: {args.lr} | dropout: {args.dropout} |char: {args.mode_char} | word: {args.mode_word} | CRF: {args.enable_crf} '| Param: {n_param} | ')
    # print(model)
    ...
    return {'T_train/epo': np.average(count_time) ,'F1_test':f1_test,'Prec_test': prec_test, 'Rec_test': rec_test}
...
def get_random_param(param_space:dict):
    '''
    随机搜索
    '''
    random.seed(random.randint(0, int(time.time()%200)))
    
    ret_param = {}
    for _para, _space in param_space.items():
        ret_param[_para]= random.sample(_space, 1)[0]
    return ret_param
...
for i in range(search_step):
    ...
    rand_search_para = get_random_param(param_space)
    args.dropout,  batch_size,  args.lr,  dim_out_word, dim_out_char,args.window_kernel = rand_search_para['dp'], rand_search_para['bs'], rand_search_para['lr'], rand_search_para['dow'],rand_search_para['doc'], rand_search_para['ks']

    metrics = _fit(args)
    
    print(f"[{i}/{search_step}] {rand_search_para}\n[gain] {metrics}")
    search_record.append((rand_search_para, metrics))
...
search_record.sort(key=functools.cmp_to_key(reversed_cmp))
#%% 

import pandas as pd

df1 = pd.DataFrame([_ for _, __ in search_record])
df2 = pd.DataFrame([__ for _, __ in search_record])
...
plot_(metric_f1, metric_prec, save=f'models/{args.mode_char}-{args.mode_word}-{args.enable_crf}.png')
