...
class BiLSTM_CRF(nn.Module):
    def __init__(self, args, word2idx, char2idx, tag2idx, glove_word=None):
        ...
        ## char 嵌入
        if self.mode_char == 'elmo':
            self.elmo_char = Elmo(args.elmo_options_file, args.elmo_weight_file, 2) # 512 dim
            for param in self.elmo_char.parameters():
                param.requires_grad = False
            self.dropout1 = nn.Dropout(args.dropout)
            # word encoder
            self.dim_in_word = self.dim_emb_word + self.dim_elmo
            
            self.lstm_word = nn.LSTM(self.dim_in_word, self.dim_out_word, batch_first=True, bidirectional=True)
    ...
    def forward(self, words_batch, chars_batch, lens_word):
        ...
        if self.mode_char == 'elmo':
            embeddings = self.elmo_char(chars_batch)
            
            word_input = embeddings['elmo_representations'][1]
            word_input = torch.autograd.Variable(word_input, requires_grad=False).to(chars_batch.device)

            # print("word_input", word_input.shape)
            # [128, 48, 1024]
            # b, word_seq, word_embd
   

        emb_words = self.embedding_word(words_batch)
        emb_words_chars = torch.cat((emb_words, word_input), dim=-1)
    ...
    def get_loss(self, words_batch, chars_batch, tags_batch, lens_batch):
        ...
        words_batch, word_raw_batch = words_batch

        if self.mode_char == 'elmo':
            chars_batch = batch_to_ids(list(word_raw_batch)).to(words_batch.device)
            # print(list(word_raw_batch))
            # print(chars_batch)
