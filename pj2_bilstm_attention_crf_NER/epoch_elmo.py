...
def train(args, model, data, optimizer):
    ...
    for batch in tqdm(data, desc='  - training', leave=False):
        words_batch, chars_batch, tags_batch, lens_batch, words_raw = map(lambda x: x.to(args.device) if hasattr(x, 'size') else x, batch)

        optimizer.zero_grad()
        loss_batch, pred_batch = model.get_loss((words_batch,words_raw), chars_batch, tags_batch, lens_batch)
        loss_batch.backward()
        optimizer.step()
    ...
    pres = precision_score(gt_all,pred_all, average='macro')
    rec = recall_score(gt_all, pred_all, average='macro', zero_division=1)
    
    loss_mean = loss_total / n_sample

    return loss_mean, (f1, pres, rec)
